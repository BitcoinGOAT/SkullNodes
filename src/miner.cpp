...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("SkullNodesMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("SkullNodesMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("SkullNodesMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("skullnodes-miner");
...
///////|| !masternodeSync.IsSynced()
        ...
        LogPrintf("Running SkullNodesMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
